...
response = (vars, req, res) ->
  if res.status == 200
    ...
    event.roaming_country = event.live.roaming_country
    event.roaming_country_iso2 = event.live.roaming_country_iso2
...
response.variables = ->
  [
    ...
    { name: 'live.location.zip', type: 'string', description: 'The 5-digit United States Postal Service ZIP Code associated with the phone number (U.S. only).'},
    ...
    { name: 'live.location.country.name', type: 'string', description: 'The Country Name associated with phone number.'},
    { name: 'live.location.country.iso2', type: 'string', description: 'The ISO 3166-1 2-letter Country Code associated with the phone number.'},
    { name: 'live.location.country.iso3', type: 'string', description: 'The ISO 3166-1 3-letter Country Code associated with the phone number.'},
    { name: 'live.location.coordinates.latitude', type: 'string', description: 'A value indicating the number of degrees of latitude of the location associated with the phone number, expressed in seven decimal digits, with five decimal places.'},
    { name: 'live.location.coordinates.longitude', type: 'string', description: 'A value indicating the number of degrees of longitude of the location associated with the phone number, expressed in eight decimal digits, with five decimal places.'},
    { name: 'live.location.timezone.name', type: 'string', description: 'A string identifying the Time Zone Name (TZ) associated with the phone number (U.S. only).'},
    { name: 'live.location.timezone.utc_offset_min', type: 'string', description: 'For U.S. domestic phone numbers, this parameter returns the UTC offset associated with the phone number.  For international phone numbers, this parameter returns the minimum UTC offset for the country associated with the phone number.'},
    { name: 'live.location.timezone.utc_offset_max', type: 'string', description: 'For international phone numbers, this parameter returns the maximum UTC offset for the country associated with the phone number. For U.S. domestic phone numbers, this parameter returns the same result as utc_offset_min.'},
